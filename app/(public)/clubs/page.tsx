      ...
      <div className="max-w-7xl mx-auto">
        ...
        {clubs.length === 0 ? (
          <div className="text-center py-20 bg-white rounded-3xl border border-slate-200 shadow-sm">
            ...
            <h3 className="text-xl font-bold text-slate-700">Belum Ada Klub</h3>
            <p className="text-slate-500 mt-2">Daftar klub saat ini masih kosong.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {clubs.map((club) => (
              <div 
                key={club.id} 
                className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col"
              >
                {/* Area Logo / Gambar Placeholder */}
                <div className="h-48 bg-slate-100 relative flex items-center justify-center border-b border-slate-100">
                  {club.logo ? (
                    <img 
                      src={club.logo} 
                      alt={`Logo ${club.name}`} 
                ...
                <div className="p-8 flex flex-col flex-grow">
                  <h2 className="text-2xl font-bold text-slate-900 mb-2">{club.name}</h2>
                  
                  <div className="space-y-3 mt-4 mb-6 text-sm text-slate-600">
                    {/* Baris Lokasi */}
...
{/* Tombol Hubungi */}
                  {club.contact ? (
                    <a 
                      ...
                      className="block w-full text-center bg-slate-900 hover:bg-slate-800 text-white font-bold py-3 px-4 rounded-xl transition-colors mt-auto"
                    >
                      Hubungi Klub
                    </a>
                  ) : (
                    <button 
                      disabled
                      className="block w-full text-center bg-slate-200 text-slate-500 font-bold py-3 px-4 rounded-xl mt-auto cursor-not-allowed"
                    >

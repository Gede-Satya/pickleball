      ...
      {categories.length > 1 && (
        <div className="sticky top-0 z-10 -mx-1 flex gap-2 overflow-x-auto px-1 py-2 bg-gradient-to-b from-white via-white/95 to-white/0">
          ...
          >
            Semua Kategori
            {totalUnassigned > 0 && (
              <span className="ml-1.5 inline-flex h-4 min-w-4 items-center justify-center rounded-full bg-[#C6E76B] px-1 text-[10px] font-bold text-[#0F172A]">
                {totalUnassigned}
              </span>
            )}
          </button>
          ...
          <CategorySection
            ...
            onGenerateBracket={() =>
              run(
                ...
                'Bracket knockout berhasil di-generate dari pool!'
              )
            }
...
function CategorySection({
  ...
  // Generate slot default: interleave pool (Pool A #1, Pool B #1, ..., Pool A #2, ...)
  // pad dengan null (BYE) sampai kelipatan 2
  ...
  const handleGenerateBracket = async () => {
    ...
    const ok = await showConfirm(
      'Generate bracket knockout dari juara & runner-up (top-2) semua pool kategori ini? Data knockout lama akan tertimpa.',
      'Generate Bracket 🏆',
      'Ya, Generate',
      'Batal'
    );
  ...
  return (
    ...
    >
      {/* Header Kategori */}
      <div className="bg-gradient-to-r from-[#0F2A3D] to-[#1C4E67] text-[#ffffff]">
        ...
        <div className="flex flex-wrap items-center gap-2 px-5 pb-4">
          <button
            onClick={onCreate}
            disabled={pending}
            className="px-3 py-1.5 rounded-lg bg-[#C6E76B] hover:bg-[#d5ef8c] disabled:opacity-50 text-[#0F172A] text-xs font-bold transition-colors"
          >
            ➕ Buat Pool
          </button>
            <button
      ...
      {/* Search per kategori */}
          ...
          {/* Modal: Atur Bagan Template (slot peringkat) */}
          {bracketOpen && (
              ...
              <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl mx-4 my-auto">
                <div className="border-b border-slate-100 px-6 py-4 flex items-center justify-between">
                  <div>
                    <h3 className="text-lg font-bold text-slate-900">
                      🗂️ Atur Bagan — {cat.label}
                    </h3>
                    <p className="text-xs text-slate-500 mt-0.5">
                      Slot berupa peringkat (angka) dulu. Setelah fase grup
                      selesai, nama peserta otomatis mengisi slot.
                    </p>
                  </div>
                  <button
                    ...
                    <div className="space-y-2">
                      {slots.map((slot, idx) => {
                        ...
                        return (
                          ...
                          >
                            <span className="w-14 shrink-0 text-xs font-bold text-slate-400">
                              Slot {idx + 1}
                            </span>
                            <select
                              value={bye ? '' : slot.poolCode}
                              onChange={(e) => {
                                ...
                                setSlots((prev) =>
                                  prev.map((s, i) =>
                                    i === idx
                                      ? code
                                        ? { poolCode: code, rank: s?.rank ?? 1 }
                                        : null
                                      : s
                                  )
                                );
                              }}
                              className="flex-1 px-2.5 py-1.5 border border-slate-200 rounded-lg text-sm font-semibold text-slate-700 focus:outline-none bg-white"
                            >
                              <option value="">— BYE (kosong) —</option>
                              {cat.pools.map((p) => (
                            ...
                            <select
                              ...
                              className="w-20 px-2.5 py-1.5 border border-slate-200 rounded-lg text-sm font-semibold text-slate-700 focus:outline-none bg-white disabled:opacity-40"
                            >
                ...
                <div className="border-t border-slate-100 px-6 py-4 flex justify-end gap-3">
                  ...
                  <button
                    onClick={saveBracket}
                    disabled={pending || cat.pools.length === 0}
                    className="px-5 py-2.5 rounded-xl bg-slate-900 text-white text-sm font-bold hover:bg-[#1C4E67] hover:text-[#ffffff] disabled:opacity-50 transition-colors"
                  >
                    💾 Simpan Bagan
                  </button>
                </div>
      ...
      <div className="bg-slate-50 border-b border-slate-200 px-4 py-2.5 flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          <button
            ...
            className="shrink-0 px-2.5 py-1 rounded-lg text-[11px] font-bold bg-slate-900 text-white hover:bg-[#1C4E67] hover:text-[#ffffff] disabled:opacity-50 transition-colors"
          >
            📊 Bagan
          </button>
          <Badge
                ...
                {m.status === 'DONE' ? (
                  ...
                  <>
                    ...
                    <button
                      ...
                      disabled={pending}
                      className="shrink-0 px-2 py-0.5 rounded-md text-[10px] font-bold bg-slate-900 text-white hover:bg-[#1C4E67] hover:text-[#ffffff] disabled:opacity-50 transition-colors"
                    >
                      🏓 Input Skor
                    </button>
                  </>

  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">Kelola Klub</h1>
          <p className="text-slate-500 mt-1">Daftar klub Pickleball yang terdaftar.</p>
        </div>
        <Link 
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-slate-100">
            {clubs.length === 0 ? (
              <tr>
                <td colSpan={4} className="p-8 text-center text-slate-400">Belum ada data klub.</td>
              </tr>
            ) : (
              clubs.map((club) => (
                <tr key={club.id} className="hover:bg-slate-50 transition">
                  <td className="p-4 font-bold text-slate-900">{club.name}</td>
                  <td className="p-4 text-slate-600">{club.location || ""}</td>
                  <td className="p-4 text-slate-600">{club.contact || "-"}</td>
                  <td className="p-4 text-right flex justify-end gap-2">
                    {/* Placeholder untuk tombol Edit nanti */}
                    <Link href={`/admin/clubs/${club.id}/edit`} className="text-blue-500 hover:text-blue-700 hover:bg-blue-50 px-3 py-1 rounded-md transition font-medium text-sm">
                      Edit
                    </Link>
